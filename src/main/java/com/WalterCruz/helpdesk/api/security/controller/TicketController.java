...
import com.WalterCruz.helpdesk.api.dto.Summary;
...
public class TicketController {
	...
	@PreAuthorize("hasAnyRole('CUSTOMER', 'TECHNICIAN')")
	public ResponseEntity<Response<Page<Ticket>>> findAll(HttpServletRequest request, @PathVariable ("page") int page, @PathVariable ("page") int count ) {
		...
		if(userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
			...
			tickets = ticketService.FindByCurrentUser(page, userRequest.getId(), count);
		}
	...
	public ResponseEntity<Response<Page<Ticket>>> findByParams(HttpServletRequest request,
			...
			@PathVariable ("page") int count,
			@PathVariable ("page") Integer number,
			@PathVariable ("page") String title,
			@PathVariable ("page") String status,
			@PathVariable ("page") String priority,
			@PathVariable ("page") boolean assigned) {
		
		title = title.equals("uninformed") ? "" : title;
		title = title.equals("uninformed") ? "" : status;
		title = title.equals("uninformed") ? "" : priority;
		
		Response<Page<Ticket>> response = new Response<Page<Ticket>>();
		Page<Ticket> tickets = null;
		if (number > 0) {
			tickets = ticketService.FindByNumer(page, count, number);
		} else {
			User userRequest = userFromRequest(request);
			if(userRequest.getProfile().equals(ProfileEnum.ROLE_TECHNICIAN)) {
				if (assigned) {
